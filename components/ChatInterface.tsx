'use client';
// components/ChatInterface.tsx — v2 Updated
// Added: Relationship System, Command Palette, Mode Detection, TTS, Context Memory
...
import { detectMode, MODE_LABELS } from '@/lib/mode-detector';
...
export default function ChatInterface() {
  ...
  const [currentMode, setCurrentMode] = useState('tool-finder');
  ...
  // Hide level-up badge after 3 seconds
  useEffect(() => {
    if (leveledUp) {
      const t = setTimeout(() => setLeveledUp(false), 3000);
  ...
  function handleCommand(cmd: string) {
    const categoryMap: Record<string, string> = {
      '/image': 'image', '/video': 'video', '/code': 'code', '/design': 'design',
    };
    if (categoryMap[cmd]) {
      setSelectedCategory(categoryMap[cmd]);
      setToolsPanelOpen(true);
      return;
    }
    if (cmd === '/favorites') {
      const favIds = linkMemory.getFavorites();
      setHighlightedToolIds(favIds);
      setToolsPanelOpen(true);
      return;
    }
    if (cmd === '/clear') {
      if (activeSessionId) {
        chatMemory.deleteSession(activeSessionId);
        handleNewChat();
      }
      return;
    }
    if (cmd === '/level') {
      const rel = relationship.get();
      const config = LEVEL_CONFIG[rel.level];
      handleSendMessage(`/level check`);
      // Show level info as JARVIS message
      if (activeSessionId) {
        const msg = chatMemory.addMessage(activeSessionId, {
          role: 'jarvis',
          content: `${config.emoji} **${config.name}** — Level ${rel.level}\n\n${rel.totalInteractions} interactions total. Keep going! 🚀`,
          timestamp: Date.now(),
        });
    ...
    if (cmd === '/export') {
      const data = chatMemory.getAllSessions();
      const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'jarvis-chats.json';
      a.click();
  ...
  const handleSendMessage = useCallback(async (content: string) => {
    ...
    const relResult = relationship.recordInteraction();
    if (relResult.leveledUp) setLeveledUp(true);
    ...
    try {
      // Get last 5 messages as context (v2: multi-turn memory)
      const currentSession = chatMemory.getSession(activeSessionId);
      ...
      // Build JARVIS response based on relationship level
      const rel = relationship.get();
      let responseText = '';
      ...
      if (topTools.length > 0) {
        ...
        // Response varies by relationship level
        if (rel.level >= 4) {
          responseText = `Bhai! "${content}" ke liye: ${toolNames} 🔥\n\nPanel mein dekh →`;
        } else if (rel.level === 3) {
          responseText = `Got it yaar! Yeh try kar: ${toolNames}\n\nPanel mein highlight kar diye →`;
        } else {
        ...
        if (intent.fallbackUsed) {
          responseText += '\n\n_(Quick match used — AI unavailable right now)_';
        }
      } else {
        responseText = rel.level >= 3
          ? `Yaar, **${intent.category}** ke liye kuch dhundh raha hoon!`
          : `Let me find the best tools for **${intent.category}**!`;
      }
      ...
      const prefs = JSON.parse(localStorage.getItem('jarvis_preferences') || '{}');
      if (prefs.ttsEnabled) {
        tts.speak(responseText.replace(/\*\*/g, '').replace(/→/g, ''));
      }
  ...
  return (
    <div className="flex h-screen bg-[#080818] overflow-hidden">
      ...
      {/* Level Up Toast (v2) */}
      {leveledUp && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 bg-[#ff1a88] text-white px-6 py-3 rounded-full shadow-lg animate-bounce text-sm font-bold">
          🎉 Level Up! {LEVEL_CONFIG[rel.level].emoji} {LEVEL_CONFIG[rel.level].name}
        </div>
      )}
      ...
      <div className={`sidebar w-64 flex-shrink-0 flex flex-col ${sidebarOpen ? 'open' : ''}`}>
        ...
        <div className="p-3 border-t border-[rgba(255,26,136,0.2)]">
          <p className="text-xs text-gray-500 text-center">🔒 All data stays in your browser</p>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-[rgba(255,26,136,0.2)] bg-[#10102a]">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">{MODE_LABELS[currentMode as keyof typeof MODE_LABELS] || '🔧 Tool Finder'}</p>
            </div>
          </div>
...
function WelcomeScreen({ onSuggestion }: { onSuggestion: (text: string) => void }) {
  const rel = relationship.get();
  const greeting = LEVEL_CONFIG[rel.level].greeting;
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center px-4 animate-fade-in">
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm max-w-sm">{greeting}</p>
        <p className="text-gray-600 text-xs mt-2">Press <kbd className="bg-white/10 px-1 rounded">/</kbd> or <kbd className="bg-white/10 px-1 rounded">Ctrl+K</kbd> for commands</p>
      </div>
